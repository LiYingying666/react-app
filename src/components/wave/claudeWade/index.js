...
// 修改createTriangleArrow函数，使其返回可单独控制的边
const createTriangleArrow = (position, direction, size) => {
    // 增大三角形尺寸
    const length = (size || 4) / 10; // 增大基础尺寸
    ...
    // 修改三角形顶点位置使其更容易看见
    const vertices = {
        left: new THREE.Vector3(0, -width / 2, 0),
        right: new THREE.Vector3(0, width / 2, 0),
        tip: new THREE.Vector3(0, 0, length),
    };

    const edges = {
        bottom: createEdge(vertices.left, vertices.right),
        leftSide: createEdge(vertices.left, vertices.tip),
        rightSide: createEdge(vertices.right, vertices.tip),
    };

    const arrow = new THREE.Group();
    arrow.add(edges.bottom);
    arrow.add(edges.leftSide);
    arrow.add(edges.rightSide);

    arrow.baseSize = { length, width };
    arrow.edges = edges;
    arrow.position.copy(position);
    arrow.visible = true; // 默认设为可见

    return arrow;
};

// 更新箭头动画
const updateArrow = (arrow, audioData, distanceFromCenter, time) => {
    if (!arrow.visible) return;

    // 分析不同频段
    const bassIntensity = audioData.slice(0, 8).reduce((a, b) => a + b, 0) / 8;
    const midIntensity = audioData.slice(8, 24).reduce((a, b) => a + b, 0) / 16;
    const trebleIntensity =
        audioData.slice(24, 32).reduce((a, b) => a + b, 0) / 8;

    // 基础缩放因子
    const baseScale = 1 - distanceFromCenter * 0.3;

    // 添加呼吸效果
    const breathe = Math.sin(time * 0.003) * 0.1;

    // 为每条边计算不同的缩放
    const bottomScale = baseScale * (1 + (bassIntensity / 255) * 0.5 + breathe);
    const leftScale = baseScale * (1 + (midIntensity / 255) * 0.5 + breathe);
    const rightScale =
        baseScale * (1 + (trebleIntensity / 255) * 0.5 + breathe);

    // 应用缩放
    arrow.edges.bottom.scale.setScalar(bottomScale);
    arrow.edges.leftSide.scale.setScalar(leftScale);
    arrow.edges.rightSide.scale.setScalar(rightScale);
};
    ...
    useEffect(() => {
        ...
        const createElevatorShaft = () => {
            ...
            corners.forEach((corner, cornerIndex) => {
                ...
                // 沿线添加箭头
                const lineLength = Math.abs(corner.z - 10);
                const arrowCount = 20; // 减少箭头数量，但增大尺寸
                const spacing = lineLength / arrowCount;

                // 在每条竖线上添加箭头
                for (let i = 0; i < arrowCount; i++) {
                    const z = corner.z + i * spacing;
                    const position = new THREE.Vector3(corner.x, corner.y, z);

                    // 根据竖线位置决定箭头朝向
                    let rotation = new THREE.Euler();
                    switch (cornerIndex) {
                        case 0: // 左下
                            rotation.set(0, 0, Math.PI * 0.25);
                            break;
                        case 1: // 右下
                            rotation.set(0, 0, -Math.PI * 0.25);
                            break;
                        case 2: // 右上
                            rotation.set(0, 0, -Math.PI * 0.75);
                            break;
                        case 3: // 左上
                            rotation.set(0, 0, Math.PI * 0.75);
                            break;
                    ...
                    const arrow = createTriangleArrow(position, null, 2);
                    arrow.setRotationFromEuler(rotation);
                    arrow.visible = true;
        ...
        const animate = () => {
            ...
            if (analyserRef.current) {
                ...
                arrows.forEach(({ arrow, distanceFromCenter }) => {
                    arrow.visible = true; // 或根据音频强度决定可见性
